 ...
 * - pull_upstream uses mindmods.exec() not host execSync
...
function makeTool(name: string, category = "vm", riskLevel: RiskLevel = "caution"): CletusTool {
  return {
    name,
...
describe("command.forbidden_patterns rule", () => {
  ...
  it("only applies to exec tool", () => {
    const request = makeRequest("write_file", { command: "rm -rf .cletus" }, "vm", "caution");
...
describe("Source code injection safety", () => {
  ...
  it("registry.ts uses execFileSync not mindmods.exec with interpolation", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync(
      new URL("../skills/registry.ts", import.meta.url).pathname.replace("/src/__tests__/../", "/src/"),
      "utf-8",
    );
    // Should NOT have template literals in mindmods.exec calls
    expect(source).not.toMatch(/mindmods\.exec\s*\(\s*`/);
  ...
  it("tools.ts pull_upstream uses mindmods.exec not host execSync", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync(
    ...
    expect(pullSection).toMatch(/ctx\.mindmods\.exec\(/);
  });
