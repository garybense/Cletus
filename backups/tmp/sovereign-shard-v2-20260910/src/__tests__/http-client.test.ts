...
describe("ResilientHttpClient", () => {
  ...
  describe("circuit breaker", () => {
    ...
    it("scopes breaker state per endpoint so one failing endpoint does not block others", async () => {
      const client = new ResilientHttpClient({
        maxRetries: 0,
        circuitBreakerThreshold: 2,
        circuitBreakerResetMs: 60_000,
      ...
      // Trip the breaker on the credits endpoint
      globalThis.fetch = vi.fn().mockRejectedValue(new Error("fail"));
      for (let i = 0; i < 2; i++) {
        await expect(
          client.request("https://api.example.com/v1/credits/balance"),
        ).rejects.toThrow("fail");
      }
      expect(client.isCircuitOpen()).toBe(true);

      // A different endpoint (sandbox exec) is unaffected
      globalThis.fetch = vi.fn().mockResolvedValue(mockResponse(200));
      const resp = await client.request(
        "https://api.example.com/v1/sandboxes/sb-123/exec",
      );
      expect(resp.status).toBe(200);

      // The credits endpoint is still open
      await expect(
        client.request("https://api.example.com/v1/credits/balance"),
      ).rejects.toThrow(CircuitOpenError);
    });

    it("normalizes resource ids so same-type operations share one breaker domain", async () => {
      const client = new ResilientHttpClient({
        maxRetries: 0,
        circuitBreakerThreshold: 2,
        circuitBreakerResetMs: 60_000,
      ...
      // Trip the breaker via exec on one sandbox
      globalThis.fetch = vi.fn().mockRejectedValue(new Error("fail"));
      for (let i = 0; i < 2; i++) {
        await expect(
          client.request("https://api.example.com/v1/sandboxes/sb-123/exec"),
        ).rejects.toThrow("fail");
      }

      // Exec on a different sandbox is the same domain and stays open
      await expect(
        client.request("https://api.example.com/v1/sandboxes/sb-456/exec"),
      ).rejects.toThrow(CircuitOpenError);

      // But file reads are a different domain and are still allowed
      globalThis.fetch = vi.fn().mockResolvedValue(mockResponse(200));
      const resp = await client.request(
        "https://api.example.com/v1/sandboxes/sb-123/files/read?path=SOUL.md",
      );
      expect(resp.status).toBe(200);
    });
  });
