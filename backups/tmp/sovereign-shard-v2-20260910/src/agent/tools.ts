...
Persistence: Enabled (long-lived context across local worker task executions)`;
    ...
    {
      name: "entelechy_create_mental_model",
      description: "Create a new mental model (pinned reflection) in Entelechy. This is a living summary document generated from a source query.",
      category: "memory" as ToolCategory,
      riskLevel: "safe" as RiskLevel,
      parameters: {
        type: "object",
        properties: {
          name: { type: "string", description: "Human-readable name for the mental model" },
          source_query: { type: "string", description: "The query to run through reflect to generate content" },
          bank_id: { type: "string", description: "Entelechy bank ID (default: 'cletus')" },
      ...
      execute: async (args) => {
        try {
          const { callEntelechyMcpTool, ENTELECHY_DEFAULT_BANK } = await import("../memory/entelechy-client.js");
          const bank_id = (args?.bank_id as string) || ENTELECHY_DEFAULT_BANK;
          const result = await callEntelechyMcpTool("create_mental_model", {
            name: args.name,
            source_query: args.source_query,
            bank_id,
            tags: args.tags,
          });
          return `Entelechy Mental Model Created:\n${JSON.stringify(result, null, 2)}`;
        } catch (e: any) {
          return `Entelechy create_mental_model error: ${e.message}`;
        }
      },
    },
    {
      name: "entelechy_refresh_mental_model",
      description: "Refresh an existing mental model by re-running its source query against the latest memories.",
      category: "memory" as ToolCategory,
      riskLevel: "safe" as RiskLevel,
      parameters: {
        type: "object",
        properties: {
          mental_model_id: { type: "string", description: "The ID of the mental model to refresh" },
          bank_id: { type: "string", description: "Entelechy bank ID (default: 'cletus')" },
        },
      ...
      execute: async (args) => {
        try {
          const { callEntelechyMcpTool, ENTELECHY_DEFAULT_BANK } = await import("../memory/entelechy-client.js");
          const bank_id = (args?.bank_id as string) || ENTELECHY_DEFAULT_BANK;
          const result = await callEntelechyMcpTool("refresh_mental_model", {
            mental_model_id: args.mental_model_id,
            bank_id,
          });
          return `Entelechy Mental Model Refresh Initiated:\n${JSON.stringify(result, null, 2)}`;
        } catch (e: any) {
          return `Entelechy refresh_mental_model error: ${e.message}`;
        }
      },
    },
    {
      name: "entelechy_distill",
      description: "Synthesize emergent wisdom from accumulated experience through the soul's identity lens.",
      category: "memory" as ToolCategory,
      riskLevel: "safe" as RiskLevel,
      parameters: {
        type: "object",
        properties: {
          query: { type: "string", description: "What to distill wisdom about (e.g. 'what have I learned about X?')" },
          bank_id: { type: "string", description: "Entelechy bank ID (default: 'cletus')" },
          budget: { type: "string", enum: ["low", "mid", "high"], description: "Search budget (default: 'high')" },
        },
        required: ["query"],
      },
      execute: async (args) => {
        try {
          const { callEntelechyMcpTool, ENTELECHY_DEFAULT_BANK } = await import("../memory/entelechy-client.js");
          const bank_id = (args?.bank_id as string) || ENTELECHY_DEFAULT_BANK;
          const result = await callEntelechyMcpTool("distill", {
            query: args.query,
            bank_id,
            budget: args.budget || "high",
          });
          return `Entelechy Distillation:\n${JSON.stringify(result, null, 2)}`;
        } catch (e: any) {
